...
if [[ -z "$(docker ps | grep "${clusterName}" || true)" ]]; then
  ...
  docker run -d --rm --name $clusterName --privileged -v hub-data:/var/lib -p 127.0.0.1:6443:6443 -p 127.0.0.1:30095:9095 -p 127.0.0.1:30080:8080 -p 127.0.0.1:30002:443 quay.io/microshift/microshift-aio:latest
